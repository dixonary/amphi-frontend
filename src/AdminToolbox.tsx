...
import { Modal, ModalBody, ModalTitle, CloseButton, Button, Spinner } from 'react-bootstrap';
...
const AdminToolbox = () => {
  ...
  >
    ...
    <ModalBody>
      ...
      {isAdmin && (<>
        {toolboxData.video && (<VideoToolbox 
...
const VideoToolbox = ({videoId, blacklistVideo, unblacklistVideo} : any) => {
  ...
  <> 
    ...
    {blacklisted && (
      <Button as="a" className="wide unblacklist" onClick={() => unblacklistVideo(videoId)}>
        Remove from blacklist
      </Button>
    )}
    {blacklisted || (
      <Button as="a" className="wide blacklist" onClick={() => blacklistVideo(videoId)}>
        Add to blacklist
      </Button>
    )}
  </>)}
...
const UserToolbox = ({userId, clearUserQueue, suspendUser, banUser, unbanUser} : any) => {
  ...
  const renderStatus = (status:string | undefined) => {
    ...
    return (<span className="amber">
      Banned until {new Date(status).toLocaleString()}{unban}
    </span>)
  }
  ...
  return (<> 
    ...
    <div className="button-row">
      <Button as="a" className="wide ban-1" onClick={() => clearUserQueue(userId)}>
        Clear queue
      </Button>
      <Button as="a" className="wide ban-2" onClick={() => suspendUser(userId, 3600)}>
        Suspend (1h)
      </Button>
      <Button as="a" className="wide ban-3" onClick={() => suspendUser(userId, 86400)}>
        Suspend (24h)
      </Button>
